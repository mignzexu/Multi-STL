...
class ScheduledSamplingWrapper(nn.Module):
    ...
    def _zeros_flag(self, batch_size, length, device, dtype):
        return torch.zeros(
            ...
            self.patch_size * self.patch_size * self.img_channel,
            device=device,
            dtype=dtype,
        )
    ...
    def _scheduled_flag(self, batch_x, eta, step):
        ...
        if step < self.configs["sampling_stop_iter"]:
            eta = max(eta - self.configs["sampling_changing_rate"], 0.0)
        else:
            eta = 0.0
        random_flip = torch.rand(batch_x.shape[0], self.aft_seq_length - 1, device=batch_x.device)
        token = (random_flip < eta).to(dtype=batch_x.dtype)
        return eta, token[:, :, None, None, None] * torch.ones_like(
            self._zeros_flag(batch_x.shape[0], self.aft_seq_length - 1, batch_x.device, batch_x.dtype)
        )

    def _reverse_scheduled_flag(self, batch_x, step):
        step_1 = self.configs["r_sampling_step_1"]
        step_2 = self.configs["r_sampling_step_2"]
        if step < step_1:
            r_eta = 0.5
            eta = 0.5
        elif step < step_2:
            r_eta = 1.0 - 0.5 * torch.exp(
                torch.tensor(-(step - step_1) / float(self.configs["r_exp_alpha"]), device=batch_x.device)
            ).item()
        ...
        r_token = (torch.rand(batch_x.shape[0], self.pre_seq_length - 1, device=batch_x.device) < r_eta).to(batch_x.dtype)
        token = (torch.rand(batch_x.shape[0], self.aft_seq_length - 1, device=batch_x.device) < eta).to(batch_x.dtype)
        flag[:, : self.pre_seq_length - 1] = r_token[:, :, None, None, None]
        flag[:, self.pre_seq_length - 1 :] = token[:, :, None, None, None]
        return flag
...
class Model(System):
    ...
    def get_model(self):
        return ScheduledSamplingWrapper(self.configs, ConvLSTM_Model)
    ...
    def _run_model(self, batch_x, batch_y, training):
        ...
        result = self.model(batch_x, batch_y, real_input_flag=flag, return_loss=False)
        return result["pred"]
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred = self._run_model(batch_x, batch_y, training=True)
    ...
    def validation_step(self, batch, batch_idx):
        batch_x, batch_y = batch
        pred = self._run_model(batch_x, batch_y, training=False)
        label = batch_y[:, :, self.label_idx[0] : self.label_idx[1], :, :]
        loss = self.criterion(pred, label)
...
def _build_debug_configs():
    ...
    return SimpleNamespace(
        total_seq=[2, 2], test_seq=2, batch_size=2, label_idx=[0, 1], in_category=["tp"], out_category=["tp"],
        img_size=[8, 8], reverse_scheduled_sampling=0, r_sampling_step_1=25000, r_sampling_step_2=50000,
        r_exp_alpha=5000, scheduled_sampling=1, sampling_stop_iter=50000, sampling_start_value=1.0,
        sampling_changing_rate=0.00002, num_hidden="4,4", filter_size=3, stride=1, patch_size=2, layer_norm=0,
        learning_rate=5e-4, weight_decay=0.0, epoch=1, std_method="z_score",
        std_params={"dataset": {"mean": [[[[0.0]]]], "std": [[[[1.0]]]]}, "metric": {"mean": [[[[[0.0]]]]], "std": [[[[[1.0]]]]]}},
        threshold=[[0.5]], metrics=["mae"], obj_dir=str(debug_dir), save_interval=1,
    )
