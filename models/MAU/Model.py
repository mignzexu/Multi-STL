...
class ScheduledSamplingWrapper(nn.Module):
    ...
    def _inference_flag(self, batch_x):
        return self._zeros_flag(batch_x.shape[0], self.total_length - self.pre_seq_length - 1, batch_x.device, batch_x.dtype)
...
class Model(System):
    ...
    def get_model(self):
        return ScheduledSamplingWrapper(self.configs, MAU_Model)
    ...
    def _run_model(self, batch_x, batch_y, training):
        ...
        return self.model(batch_x, batch_y, real_input_flag=flag, return_loss=False)["pred"]
...
def _build_debug_configs():
    ...
    return SimpleNamespace(total_seq=[2, 2], test_seq=2, batch_size=2, label_idx=[0, 1], in_category=["tp"], out_category=["tp"], img_size=[8, 8], scheduled_sampling=1, sampling_stop_iter=50000, sampling_start_value=1.0, sampling_changing_rate=0.00002, num_hidden="4,4", filter_size=3, stride=1, patch_size=1, layer_norm=0, sr_size=2, tau=2, cell_mode="normal", model_mode="normal", learning_rate=5e-4, weight_decay=0.0, epoch=1, std_method="z_score", std_params={"dataset": {"mean": [[[[0.0]]]], "std": [[[[1.0]]]]}, "metric": {"mean": [[[[[0.0]]]]], "std": [[[[[1.0]]]]]}}, threshold=[[0.5]], metrics=["mae"], obj_dir=str(debug_dir), save_interval=1)
