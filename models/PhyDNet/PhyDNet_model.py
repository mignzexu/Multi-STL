...
class Main(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.device = device
        self.model = PhyDNet_Model(self.configs, self.device)
    ...
    def forward(self, batch_x, batch_y=None):
        for ei in range(self.pre_seq_length - 1):
            _, _, output_image, _, _  = \
                self.model.encoder(batch_x[:,ei,:,:,:], (ei==0))
...
class PhyDNet_Model(nn.Module):
    ...
    def __init__(self, configs, device, **kwargs):
        ...
        self.phycell = PhyCell(input_shape=input_shape, input_dim=64, F_hidden_dims=[49],
                               n_layers=1, kernel_size=(7,7), device=self.device)
        self.convcell = PhyD_ConvLSTM(input_shape=input_shape, input_dim=64, hidden_dims=[128,128,64],
                                      n_layers=3, kernel_size=(3,3), device=self.device)
    ...
    def forward(self, input_tensor, target_tensor, constraints, teacher_forcing_ratio=0.0):
        ...
        return {"pred" : predictions, "loss": loss}
