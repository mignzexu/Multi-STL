...
if __package__ in (None, ""):
    ...
    TAU_Model = importlib.import_module("TAU_model").TAU_Model
else:
...
class Model(System):
    ...
    def get_model(self):
        return TAU_Model(vars(self.configs))
    ...
    def diff_div_reg(self, pred_y, batch_y, tau=0.1, eps=1e-12):
        batch_size, timesteps, channels = pred_y.shape[:3]
        if timesteps <= 2:
            return pred_y.new_zeros(())
        gap_pred_y = (pred_y[:, 1:] - pred_y[:, :-1]).reshape(batch_size, timesteps - 1, -1)
        gap_batch_y = (batch_y[:, 1:] - batch_y[:, :-1]).reshape(batch_size, timesteps - 1, -1)
    ...
    def _loss(self, pred, label):
        return self.criterion(pred, label) + self.alpha * self.diff_div_reg(pred, label)
